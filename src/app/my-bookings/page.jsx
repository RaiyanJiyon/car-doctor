...
const BookingsPage = () => {
    ...
    const fetchBookings = async () => {
        try {
            setLoading(true);
            const response = await fetch(`http://localhost:3000/my-bookings/api/bookings?email=${encodeURIComponent(session.user.email)}`);
    ...
    const handleCancelBooking = async (id) => {
        try {
            const response = await fetch(`http://localhost:3000/my-bookings/api/booking-delete/${id}`, {
                method: 'DELETE',
            });
                        ...
                        {bookings.map(booking => (
                            <tr key={booking._id} className="bg-white border-b hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 flex gap-4">
                                    ...
                                    <button onClick={() => handleCancelBooking(booking._id)} className="text-red-500 hover:underline">Cancel</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
