...
const CheckoutPage = ({ params }) => {
    ...
    const onSubmit = async (data) => {
        try {
            const response = await fetch(`http://localhost:3000/checkout/api/${params._id}`, {
                method: 'POST',
                headers: {
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                />
                                {errors.firstName && <span className="text-primary">{errors.firstName.message}</span>}
                            </div>
                            <div>
                                <input
                        ...
                        <div>
                            <textarea
                                ...
                                className="w-full p-3 rounded-lg focus:ring-2 focus:ring-red-500 focus:outline-none"
                            />
                            {errors.message && <span className="text-primary">{errors.message}</span>}
                        </div>
